package net.guidebookbutton.screen;
...
import net.guidebookbutton.util.ScreenCompat;
import vazkii.patchouli.api.PatchouliAPI;

@Environment(EnvType.CLIENT)
public class GuidebookButtonScreen extends Screen {

    private List<Object> list = new ArrayList<>();
    private int page = 0;

    public GuidebookButtonScreen(List<Object> list) {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        for (; u < this.list.size(); u += 3) {
            if (this.isPointWithinBounds(-62, -60 + ((u / 3) - this.page * 6) * 20, 110, 18, (double) mouseX, (double) mouseY)) {
                if (GuidebookButtonMain.isBYGLoaded && ((Identifier) this.list.get(u)).equals(new Identifier("byg", "biomepedia"))) {
                    ScreenCompat.setBiomepediaScreen(this.client);
                } else if ( FabricLoader.getInstance().isModLoaded("ftbquests") && ((Identifier) this.list.get(u)).equals(new Identifier("ftbquests", "book"))) {
                    ScreenCompat.setFTBQuestsScreen(this.client);
                }{
